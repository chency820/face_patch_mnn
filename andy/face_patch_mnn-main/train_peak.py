...
import matplotlib_pyplot
matplotlib_pyplot.use('TkAgg')
...
parser.add_argument('--fold', default=0, type=int, help='k fold number')
...
trainset = MyDataset(split='Training', fold=opt.fold, transform=None)
testset = MyDataset(split='Testing', fold=opt.fold, transform=None)
trainloader = torch.utils.data.DataLoader(
    trainset, batch_size=50, shuffle=True, num_workers=0)
...
def test(epoch):
    ...
    if epoch == total_epoch-1:
        logging.info('best_test_acc: {0},best_epoch:{1}'.format(best_Test_acc,best_epoch))
    '''
    if epoch == total_epoch - 1:
